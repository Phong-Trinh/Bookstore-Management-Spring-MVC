...
public class SupplierController {
	...
	public List<NCCEntity> findSuppliers(String id, String infor, Integer pageAt, Integer pageCount) {
		Session session = factory.getCurrentSession();
		int countPara = -1;
		...
		List<NCCEntity> list;
		int takeNumber = pageSize;
		if (pageCount % pageSize != 0)
		{
			takeNumber = pageCount % pageSize;
		}
		try {
			list = query.setFirstResult(pageCount-(pageAt * pageSize)+1).setMaxResults(takeNumber).list();
	...
	public List<NCCEntity> getSuppliers(Integer pageAt, Integer pageCount) {
		Session session = factory.getCurrentSession();
		String hql = "FROM NCCEntity";
		Query query = session.createQuery(hql);
		int takeNumber = pageSize;
		if (pageCount % pageSize != 0 && pageAt == (int) pageCount / pageSize)
		{
			takeNumber = pageCount % pageSize;
		}
		int skipNumber = (pageCount-((pageAt+1) * pageSize) );
		if (skipNumber < 0)
		{
			skipNumber = 0;
		}
		System.out.printf(takeNumber + "eeee" + pageCount + "eeee" + skipNumber);
		List<NCCEntity> list = query.setFirstResult(skipNumber).setMaxResults(takeNumber).list();
		Collections.reverse(list);
		return list;
	}
	...
	public String mainCustomer(HttpServletRequest request, Model model, HttpSession session,
		...
		List<NCCEntity> suppliers;
		long pageCount = 0;
		if (idStr.isEmpty() && infor.isEmpty()) {
			pageCount = this.getPageCount();
			suppliers = this.getSuppliers(page, (int) pageCount);
		} else {
			pageCount = this.getPageCount(idStr, infor);
			suppliers = this.findSuppliers(idStr, infor, page,(int) pageCount);
		}
