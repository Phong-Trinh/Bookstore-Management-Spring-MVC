...
public class CategoryController {
	...
	public String getPhieuNhap(@RequestBody PhieuNhapAjax data) throws InterruptedException {
		...
		for (int i = 0; i < data.getCtpn().size(); i++) {
			...
			if (i < ctpnArrExist.size() && ctpnArrExist.size() > 0
				...
				CTPNEntity CTTonTai = this.getCTPNById(ctpnArrExist.get(i).getId());
		...
		if (updated == true) {
			...
			for (int i = 0; i < ctpnArrExist.size(); i++) {
				if (!CheckExistCTPN(data.getCtpn(), ctpnArrExist.get(i))) {
					...
					errorCodePN = this.deleteCTPN(ctpnDelete);
				}
			}
		}
	...
	public String productLinkDelete(HttpServletRequest request, Model model, Principal principal,
		...
		// Tim phieu nhap
		PhieuNhapEntity phieuNhapDelete = (PhieuNhapEntity) sessionDB.get(PhieuNhapEntity.class, id);

		Transaction t = sessionDB.beginTransaction();
		try {
			sessionDB.delete(phieuNhapDelete);
			t.commit();
			errorCode = 1;
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (errorCode != 0) {
			webMessage.setMessageType("Thành công");
			webMessage.setMessage("Xóa phiếu nhập thành công !");
		} else {
			webMessage.setMessageType("Thất bại");
			webMessage.setMessage("Xóa phiếu nhập thất bại !");
		}
		session.setAttribute("webMessage", webMessage);
		return "redirect:/category/index2.htm";
	}
